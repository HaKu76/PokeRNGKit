/*
 * PokeRNGKit Gen VI TinyFinder Honey Wild WebAssembly bridge.
...
constexpr std::uint32_t maxTiming = 1000;
...
thread_local std::vector<gen6tinyhoneyResult> results;
...
thread_local std::uint32_t honeyDelay = 0;
thread_local std::uint32_t party = 0;
thread_local std::uint32_t bagAdvances = 0;
thread_local bool oras = false;
thread_local bool emulator = false;
thread_local std::uint32_t slotType = 0;
              ...
              bool &shortHappened, std::uint32_t target)
{
    while (advances < target)
    {
        ...
        else
        {
            ...
            {
                advances += shortCooldown(rng.next());
                shortHappened = true;
            }
        }
...
Generated generate()
{
    ...
    const auto resolvedBagAdvances = bagAdvances == 0U ? (oras ? 15U : 27U) : bagAdvances;
    rng.advance(party * 3U + resolvedBagAdvances - 1U);
    std::uint32_t advances = blinkRand;
    if (oras && advances >= 4U) advances -= 4U;
    bool shortHappened = true;
    result.actualDelay = blinkRand + honeyDelay;
    ...
    constexpr std::array<std::uint32_t, 12> normalDistribution = {10, 10, 10, 10, 10, 10, 10, 10, 10, 5, 4, 1};
    constexpr std::array<std::uint32_t, 5> waterDistribution = {50, 30, 15, 4, 1};
    const auto activeSlotCount = slotType == 4U ? 5U : 12U;
    for (std::uint32_t index = 0; index < activeSlotCount; ++index)
    {
        const auto weight = slotType == 4U ? waterDistribution[index] : normalDistribution[index];
        if (slotRandom < weight)
        {
            result.slot = index + 1;
            break;
        }
    ...
    result.risky = closeTo(result.actualDelay, result.timeline);
    return result;
}
...
bool valid(const std::uint32_t *request)
{
    if (!request || request[0] > 1 || request[7] > maxIndex ||
        request[7] < request[6] || request[8] > maxTiming || request[9] > maxTiming ||
        request[10] < 1 || request[10] > 6 || request[11] > 100 ||
        request[12] > 1 || request[13] > 1 || (request[14] != 0 && request[14] != 4) ||
        request[15] > 1 || request[16] > 1 || request[17] > 4 || request[18] > 0xfff ||
        request[19] == 0 || request[19] > 100000 ||
        static_cast<std::uint64_t>(request[7]) - request[6] + 1 > maxTasks)
        return false;
    for (std::uint32_t index = 0; index < slotCount; ++index)
        if (request[20 + index] > 721 || request[32 + index] == 0 || request[32 + index] > 100)
            return false;
    return true;
}
}

extern "C" std::uint32_t gen6tinyhoney_api_version() { return apiVersion; }

extern "C" std::uint32_t gen6tinyhoney_begin(const std::uint32_t *request)
{
    results.clear();
    ...
    maxValue = request[7];
    blinkRand = request[8];
    honeyDelay = request[9];
    party = request[10];
    bagAdvances = request[11];
    oras = request[12] != 0;
    emulator = request[13] != 0;
    slotType = request[14];
    synchronizeOnly = request[15] != 0;
    safeOnly = request[16] != 0;
    fluteFilter = request[17];
    slotMask = request[18];
    resultLimit = request[19];
    for (std::uint32_t index = 0; index < slotCount; ++index)
    {
        species[index] = request[20 + index];
        levels[index] = request[32 + index];
    }
...
extern "C" std::uint32_t gen6tinyhoney_step(std::uint32_t maximumStates)
{
    results.clear();
    stepProcessed = 0;
    if (!initialized)
    ...
    while (stepProcessed < limit && !gen6tinyhoney_done())
    {
...
extern "C" std::uintptr_t gen6tinyhoney_result_ptr()
{
    return results.empty() ? 0 : reinterpret_cast<std::uintptr_t>(results.data());
}
extern "C" std::uint32_t gen6tinyhoney_result_count() { return static_cast<std::uint32_t>(results.size()); }
extern "C" std::uint32_t gen6tinyhoney_step_processed() { return stepProcessed; }
extern "C" std::uint32_t gen6tinyhoney_total_processed() { return totalProcessed; }
extern "C" std::uint32_t gen6tinyhoney_done() { return initialized && currentIndex > maxValue ? 1U : 0U; }
extern "C" std::uint32_t gen6tinyhoney_limit_reached() { return limited ? 1U : 0U; }
extern "C" std::uint32_t gen6tinyhoney_last_error() { return error; }
