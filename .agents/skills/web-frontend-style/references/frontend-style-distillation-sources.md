...
当前状态：已完成 32 组样本的第一轮证据记录与第二轮交叉蒸馏。本文保留来源、观察证据、平台标签和许可边界；归一化后的字体、圆角、布局、交互、主题和反 AI 规则分别维护在同目录的专项文档中，后续样本可继续追加。
...
## 32. PokeRNGKit 工具网页的低成本玻璃 Chrome

来源：

- 本地仓库：`C:\Users\Hakuhiro\Documents\GitHub\PokeHero`
- GitHub：https://github.com/HaKu76/PokeRNGKit

平台标签：

- 主适配：PC 浏览器、桌面优先响应式 Web 工具和数据工作台。
- 可迁移：平板 Web、PWA/WebView、需要低 GPU 成本的桌面工具壳。
- 不适用：依赖壁纸透出的沉浸式玻璃主页、需要强烈背景折射的展示页、原生 Android/iOS 材质实现。

观察范围：React/Vite 应用的主题 Token、固定顶栏、模块抽屉、状态标签、主题/菜单按钮、悬浮工具栏、弹层遮罩、内容面板、表格和明暗主题。样式主要位于 `src/styles.css`；本轮只进行源码级观察，没有修改 PokeRNGKit，也没有运行构建或浏览器验收。

蒸馏结论：

- **它是玻璃观感，不是真实背景模糊**：源码没有使用 `backdrop-filter`。顶栏和侧栏先使用不透明的深蓝实体底（浅色主题约 `#102344/#152b4e`，深色主题约 `#0d1c37/#111f3a`），再把内部按钮和标签做成 `rgba(255,255,255,.06-.08)` 的浅亮薄层，因此在低成本下形成类似磨砂玻璃控件的观感。
- **半透明只放在 chrome 内部**：状态标签、主题按钮、菜单按钮和抽屉关闭按钮使用约 `.06` 的白色填充与 `.20-.28` 的浅色边框；hover 约 `.045`，选中层使用冷蓝 `rgba(123,167,241,.16-.20)`。正文面板、表单、表格、下拉菜单和悬浮工具面板仍使用实体 `surface`，避免数据内容被背景干扰。
- **边框、阴影和遮罩各自承担一种深度**：顶栏底边约 `.24`，控制边框约 `.22-.28`，侧栏边界约 `.20`；顶栏用向下阴影，抽屉用向右阴影，浮层使用更深、更大的独立阴影。页面遮罩按层级约为 `.28`（非阻塞悬浮工具）、`.46`（模块抽屉）和 `.66`（模态对话框），不能用同一透明度覆盖所有层级。
- **几何稳定后材质才成立**：顶栏约 `64px`，chrome 控件 `40px`，抽屉行至少 `46px`，悬浮工具按钮 `48px`；标签和按钮圆角 `8-10px`，抽屉/工具栏 `14px`，内容卡 `16px`，浮层 `18px`。材质变化只改变填充、边框和阴影，不让 hover/active 改变控件尺寸。
- **状态通过局部增亮而不是全局发光**：普通状态保持低透明浅层；hover 提升表面或边框，active 使用冷蓝填充或 `3px` 内侧状态条，focus 使用独立 ring。强调色只表示选中、焦点或语义状态，不让普通文字和边框同时变成同一饱和色。
- **明暗主题保持同一材质逻辑**：两套主题只替换背景、surface、line、text、brand 与 shadow Token；组件尺寸、层级和交互时序不变。深色面板仍是可读的实体蓝黑表面，而不是继续降低 alpha。
- **动效服务于层级建立**：hover 约 `140ms`，抽屉/浮层约 `220ms`，使用快速收尾的 ease-out；抽屉平移、遮罩淡入和浮层进入同步，避免给每个半透明控件添加持续漂浮、光晕或模糊动画。

可执行配方：先做“实体着色底 + 低透明内部控件 + 分级遮罩 + 定向阴影”，只有产品确实需要透出壁纸时，才把外层底色改为高不透明玻璃并增加受限 blur。即使浏览器不支持 blur，这套配方也必须维持完整层级、对比度和交互状态。

适用边界：适合作为工具类 Web 的顶栏、侧栏、状态区、页脚和悬浮命令 chrome；不要把 `.06` 的内部控件透明度误用于正文卡片，也不要把这套“拟玻璃”描述成来源项目实际使用了背景模糊。

许可/归属：PokeRNGKit 仓库使用 GPL-3.0。HakuStyle 仅记录用户本地项目中可迁移的视觉规律和数值区间，不复制其组件实现、业务代码、图标、Logo、宝可梦素材或其他受保护内容；若直接复用源码，应遵守 GPL-3.0 及相关第三方素材许可。
...
| 32 PokeRNGKit 低成本玻璃 Chrome | PC 浏览器、桌面优先响应式 Web 工具 | 平板 Web、PWA/WebView           | 原项目无 `backdrop-filter`；半透明仅用于实体 chrome 内部，不扩散到正文面板 |
...
- `source-contribution-index.md`：逐项说明全部 32 个来源在第二次蒸馏中承担的具体职责，保证来源被吸收但不会在单一页面中无条件堆叠。

## 当前项目状态

- 已记录用户目前提供的全部 32 组来源，并完成平台适配标签和第二次交叉蒸馏。
