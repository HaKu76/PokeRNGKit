...
`995px` 时使用 `360px / 260px / 自适应` 三列，把乱数信息、筛选项、结果放在同一行；`721..994px`
时先保留乱数信息/筛选项双列并把结果移到下一行，`720px` 以下改为单列。Searcher 继续使用 `440px`
输入区与自适应结果区；桌面同一行的面板统一拉齐高度，窄视口单列时按内容高度排列。
