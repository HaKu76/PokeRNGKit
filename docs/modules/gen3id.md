...
Generator 使用左侧 `364px` 的乱数信息与右侧自适应筛选区域；Searcher 使用 `440px` 输入区与
自适应结果区。窄视口时两个工作区都改为单列。输入、分段标签、下拉触发器、下拉选项和操作按钮均
使用模块局部的 `30px` 紧凑密度，结果状态、提示和指标放入标题区域，最后一列不绘制多余右边界。

模块布局与密度位于 `src/features/id/Gen3IdPanel.css`，不写入全局样式。共享 Select、MultiCheckSelect
和自动完成菜单把实际渲染后的菜单高度传给 `useMenuPlacement`，因此菜单只会在下方空间不足时向上展开，
并保持与触发器相同的可用宽度。该界面调整不改变 Generator/Searcher 请求、过滤或 Wasm 输出语义。

2026-08-27 已在外部 Chrome 的本地 UI 预览中核对桌面、`900×800` 和 `390×844`：无横向溢出，目标 PID
菜单宽度与 `248px` 触发器一致，触发器和选项均为 `30px`。这是界面工程证据，不替代生产页面算法回归或
项目所有者验收。
