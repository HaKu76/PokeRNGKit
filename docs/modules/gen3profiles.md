...
- 存档信息、个体值计算器和遇敌查询默认收起，以统一的 52px 纵向工具轨固定在右下角；桌面面板在工具轨左侧独立打开，窄屏面板在工具轨上方打开，任意时刻只允许展开一个工具。
- 点击工具轨按钮切换面板；点击面板外部、按 `Escape` 或点击面板关闭按钮会收起，`Escape` 与关闭按钮会把焦点归还原触发按钮。存档管理弹窗仍独立于工具面板。
- `Manager` 打开完整管理界面，提供新建、编辑、复制、删除、导入、导出和清除。
- 当前存档名显示在面板标题中；收起态只保留稳定工具按钮，减少对主要表单的遮挡。
