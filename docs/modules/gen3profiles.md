...
应用全局右下角显示小型存档信息悬浮窗，在 ID 与 Static 工作区切换时保持可用：

- 首次打开默认收起；展开时可切换兼容存档，并查看游戏、TID 和 SID。
- 展开后点击悬浮窗以外的页面区域会自动收起，并同步保存折叠状态；打开管理器时不会关闭或中断管理弹窗。
- `Manager` 打开完整管理界面，提供新建、编辑、复制、删除、导入、导出和清除。
- 折叠后保留当前存档名，不覆盖主要表单。
- 展开状态保存在 `pokerngkit-gen3-profile-panel-expanded`，存档内容仍只由 repository 管理。
