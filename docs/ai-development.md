...
模块数量增长后，开发循环应优先采用增量检查：只运行受影响模块的测试、只对本轮修改文件执行 lint，并只构建受影响的 Worker/Wasm 目标。提交或发布前仍必须运行与 CI 一致的完整验证，不能用定向结果替代全量门槛。

这套分层方法适用于多数 TypeScript、Vite、Vitest 和 Worker/Wasm 仓库；具体测试池、并发数、Worker 数量、缓存策略和构建拆分取决于仓库与操作系统，必须用实际耗时、稳定性和资源占用重新基准测试。当前 PokeRNGKit 的实测参数和命令保留在 [`docs/progress.md`](progress.md) 的“工程验证耗时优化决策”中。
...
- 结果表首次出现数据时，第一条记录必须直接贴合表头；虚拟列表的顶部占位只能用于表头，不得重复计入首条数据。
- 同一行的输入框、下拉框、只读值和操作按钮必须共享控件高度与底部基线；辅助文本不得只出现在某一列而撑高同行。
- 只显示 PokeFinder 上游实际存在的标签、版本和状态；没有上游来源的推断性说明不得加入产品界面。
- 标记为全局工具的功能必须由工具自身选择世代或游戏，不能根据当前左侧 RNG 模块自动限代；侧栏模块按世代分组时，全局工具仍保持单一入口。
- 新增或修改虚拟结果表时，必须横查 Initial Seed、Static、Wild、Egg 和 Gen4 Static 的表头偏移与首行定位，避免同类空行问题重复进入生产包。
