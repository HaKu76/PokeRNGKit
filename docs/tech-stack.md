...
> - 状态：第三世代既有模块与第四世代 Static 已接入独立 Wasm/Worker；最新上游合并后的工程与部署回归待完成
> - 更新日期：2026-08-12
> - 当前范围：第三世代 ID、Initial Seed Finder、Static/Wild Generator/Searcher、IVs to PID、Egg Generator，第四世代 Static Generator/Searcher，两代独立存档与个体值计算器，以及 Encounter Lookup
> - 包管理器：npm
  ...
  `-- production -> module-specific Worker Pool
        ...
        `-- Gen4StaticSearcherWorkerPool -+-> gen4static.mjs + gen4static.wasm
                                           ...
                                           `-- narrow C ABI bridges
                                            `-- PokeFinder 4.3.2 Gen III / Gen IV Static rules
```
...
当前有 ID、Initial Seed、G3 Static、Wild、IVs to PID、Egg 与 G4 Static 七个 RNG 工作区，状态仍由各自 React 组件的 `useState`、`useMemo` 和明确的搜索引擎实例管理。存档信息分别由 `useGen3Profiles`、`useGen4Profiles` 与各自 repository 层持有，不引入 Zustand、Redux 或其他全局状态框架；Encounter Lookup 使用独立本地静态数据，不进入 RNG Worker 状态。
...
- UI 预览：ID、Initial Seed、G3/G4 Static、Wild、IVs to PID 与 Egg 各自使用同一请求边界的确定性样例引擎，不读取或生成 Wasm；预览结果只用于界面交互验收。
...
`src/features/shared/rngModuleContract.ts` 保留跨世代的 manifest、Worker 信封和第四世代模块标识。`gen4static` 已注册 API v1、独立产物、导航、Worker Pool 和运行时；`gen4id` 与 `gen4wild` 仍只有 reservation。实施边界见[第四世代扩展接口与 AI 交接](gen4-development.md)。
...
当前 `gen3id` API 版本为 2，`gen3initialseed` API 版本为 1，`gen3static` API 版本为 3，`gen3wild` API 版本为 3，`gen3ivtopid` API 版本为 1，`gen3egg` API 版本为 1，`gen4static` API 版本为 1。ID C ABI 为：
...
- 每次 C ABI 调用最多处理 100,000 个 Generator 状态；Egg 以 Held 范围分片且每次最多处理 100,000 个 Held/Pickup/Redraw 组合；Wild Searcher 的 TypeScript 分片上限为 10,000 个 IV 组合；G4 Static Searcher 的 TypeScript 分片上限为 500 个 IV 组合，C ABI 保留 100,000 的防御上限。
...
- ID、Initial Seed、Static、Wild、IVs to PID 与 Egg 使用相同的消息信封原则，但保留独立 TypeScript 类型、Worker 文件和 API 版本，不使用未加区分的通用 payload。Initial Seed 以 `rs-ids` 与 `target` 区分操作，只有目标 Seed 反推携带 `chunkIndex`；IVs to PID 每次输入是一个有限候选任务，不拆分 chunk；Egg 以 Held Advances 分片，保留 Pickup 范围和 Redraws 作为每个分片的完整请求输入。
...
- 上游文件保留原版权与 GPL 头。
- PokeRNGKit bridge 使用独立文件和 `gen3id_*`、`gen3initialseed_*`、`gen3static_*`、`gen3wild_*`、`gen3ivtopid_*`、`gen3egg_*`、`gen4static_*` 前缀。
...
后续模块沿用同一结构。G3 Static、G4 Static 与 Wild 的 Generator/Searcher 各自共享所属版本化模块，但使用独立请求、Pool 和任务生命周期；不要把 `gen4id`、`gen4wild` 或其他世代逻辑塞进现有模块。
...
- **C++ 原生夹具**：ID 三种模式，Initial Seed 的 RS TID/SID 固定候选，G3 Static Method 1/4、Searcher 反向恢复、游走缺陷，Wild Route 111 Generator/Searcher，IVs to PID Channel/Method 2，Egg Emerald/RSFRLG，以及 G4 Static Method 1/J/K、Synchronize、Cute Charm 与 Searcher 固定结果。
- **TypeScript 单元测试**：ID、G3/G4 Static、Wild、IVs to PID、Egg 输入边界，Generator/Searcher 分片、固定宽度结果解码、觉醒力量、输入规范化、主题和红蓝宝石 Seed 推导。
- **持久化单元测试**：G3/G4 存档 JSON schema、合并边界、IndexedDB 主存储抽象与 localStorage 兜底。
- **UI 预览引擎测试**：ID、G3/G4 Static、Wild Generator/Searcher、IVs to PID 与 Egg Generator 的确定性样例、进度和取消。Initial Seed 预览引擎已实现，组件测试待后续补充。
...
- Playwright：GitHub Pages 子路径、ID/Initial Seed/G3 Static/Wild/IVs to PID/Egg/G4 Static 冒烟、两代独立悬浮工具、离线重载和移动视口。
...
当前 `gen3initialseed`、`gen3ivtopid` 与 `gen3egg` 进入部署页面算法回归前必须完成：
