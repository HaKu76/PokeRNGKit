...
- 当前 Windows portable job 使用同一份 `production-dist`，通过 7-Zip SFX 包装静态文件和仅监听回环地址的 PowerShell 启动器；该产物仅作为临时开发验收包。最终交付必须改为直接构建的桌面原生 EXE，目标体验参照 PokeFinder / 3DSRNGTool；原生桌面壳、安装方式和 Actions 构建链尚待确定。
