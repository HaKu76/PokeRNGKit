...
当前按 PokeFinder 功能模块逐个落地。第三世代 ID、Initial Seed、Static/Wild Generator/Searcher、IVs to PID、Egg、存档信息、个体值计算器和全世代 Encounter Lookup 已进入 Git 基线；当前分支新增第四世代 Static Generator/Searcher、独立 G4 存档和独立 G4 个体值计算器。
...
## 8.2 当前功能需求：`gen3ivtopid` IVs to PID
...
## 8.4 当前功能需求：`encounterlookup` 遇敌查询
...
- **FR-G4STATIC-08** G3/G4 页面均保留 Encounter Lookup；展开时只与当前页面对应的个体值计算器互斥。
...
3. `npm run wasm:test:native` 通过 ID Generator 三种模式、RS ID Searcher SID/PID/无解、Initial Seed RS ID 固定候选、G3 Static Method 1/4、Searcher 反向恢复、游走缺陷、Wild Route 111 Generator/Searcher、IVs to PID Channel/Method 2、Egg Emerald/RSFRLG，以及 G4 Static Method 1/J/K、Synchronize、Cute Charm、Searcher 与错误边界夹具。
4. `npm run wasm:build` 生成 `gen3id`、`gen3initialseed`、`gen3static`、`gen3wild`、`gen3ivtopid`、`gen3egg` 与 `gen4static` 的 MJS/Wasm 产物。
...
11. G4 Static 使用已记录固定输入核对 Method 1/J/K、Synchronize、Cute Charm、`Max Advances + 1`、Searcher Seed、PID 和六项 IV；同时确认 G3/G4 存档与个体值计算器相互独立。
12. GitHub Pages 在线加载 7 个 Worker/Wasm 模块，控制台无资源、API 握手或 Worker 错误。
13. 记录部署 URL、对应 commit/Actions run、浏览器版本、输入、预期、实际结果和未覆盖项。
...
- **阶段 7：`gen4static` Static Generator/Searcher** - 第四世代 Method 1/J/K、独立 G4 存档与个体值计算器、Wasm/Worker 和算法文档（当前分支，待 Actions、部署回归与最终验收）。
