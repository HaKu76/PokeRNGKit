...
- **FR-G7EGG-01** 提供 3DSRNGTool Gen VII Egg RNG 的 Frame Range、Egg Number 与 Shortest Path 三种工作流，支持四字 TinyMT 状态、TSV/TRV、双亲 IV/道具/特性/百变怪、性别比、异国孵化、闪耀护符、尼多型、同图鉴、其他 TSV 与闪数提醒。
- **FR-G7EGG-02** 输入必须匹配上游控件与 Core 类型：TinyMT 为四个 8 位十六进制字，TSV `0..4095`，TRV `0..F`，帧与目标帧 `0..1,000,000,000`，蛋数 `1..10,000`，亲代与筛选 IV `0..31`，完美 IV 数量 `0..6`；Shortest Path 的浏览器执行上限为 `5,000,000`。
- **FR-G7EGG-03** 领域校验必须保持百变怪、性别比、尼多型、同图鉴、其他 TSV、闪数提醒与不变之石的跨字段行为，并在 TypeScript、Worker 与 Wasm ABI 入口使用相同规则。
- **FR-G7EGG-04** TinyMT、`Egg7` 生成顺序、遗传、PID 重抽、Frame Range、Egg Number 与最短 Accept / Reject 路径只在独立 `gen7egg` Wasm API v1 中执行。模块使用 187-word 请求、20-word 结果、`begin()` / `step()` 连续会话与单 Dedicated Worker，不使用线程共享内存。
- **FR-G7EGG-05** 结果保留 Frame、Egg Number、当前/领取后 TinyMT 状态、Random Number、EC、PID、六项 IV、遗传来源、Nature、Ability、Gender、Hidden Power、Shiny、Ball、Parent Nature、Action 与 Frames Used；提供进度、取消、虚拟滚动、排序、CSV、清空、状态回写、错误、空结果与结果上限状态。
...
## 8.36 当前功能需求：`gen7event`

- **FR-G7EVENT-01** 提供 3DSRNGTool Gen VII `Event RNG` 连续帧工作流，支持 Sun、Moon、Ultra Sun、Ultra Moon，以及本地 `.wc7` / `.wc7full` 配信卡导入。
- **FR-G7EVENT-02** 输入必须匹配上游 WinForms 与 Core 类型：Seed 为 8 位十六进制；SM 起始帧 `418`，USUM 起始帧 `478`；帧上游上限 `1,000,000,000`、浏览器绝对帧保护 `5,000,000`；TSV `0..4095`、TRV `0..F`、NPC `0..100`、Delay `0..4000`、TID/SID `0..65535`、EC/PID 为 32 位十六进制。
- **FR-G7EVENT-03** 支持物种、形态、等级、六项固定 IV、保底随机 V 数、Ability/Nature/Gender 锁定、Your ID、Egg、No Dex Entry、Other Information 与 Random / Nonshiny / Shiny / Specified PID Type；固定 IV 数量加非零保底随机 V 数不得超过 5。
- **FR-G7EVENT-04** SFMT64、NPC 模型、`Event7.Delay()`、丢弃生成、`Event7.Generate()` 和 `Search7_Normal()` 只在独立 `gen7event` Wasm API v1 中执行。模块使用 58-word 请求、9-word 结果、`begin()` / `step()` 连续会话和单 Dedicated Worker。
- **FR-G7EVENT-05** 结果保留 Frame、Realtime、Random Number、EC、PID、六项 IV、Nature、Ability、Gender、Hidden Power、Shiny、Blink、Delay、PSV 与 PRV；提供完整筛选、进度、取消、100000 行结果上限、虚拟滚动、排序、CSV、清空、错误、空结果与结果上限状态。

完整输入限制、Wonder Card 字段、生成顺序、58/9-word ABI、固定夹具和上游文件见 [Gen 7 Event](modules/gen7event.md)。

## 8.37 当前功能需求：`gen7main`

- **FR-G7MAIN-01** 提供读档指针序列反查 Seed、QR 指针序列定位、ID RNG Seed 反查和 Time Calculator，支持 Sun、Moon、Ultra Sun 与 Ultra Moon。
- **FR-G7MAIN-02** Clock 每项为 `0..16`；读档、ID、QR 最少分别需要 8、9、2 项；Seed 为 32 位十六进制且空值按 `0`；QR 与时间计算帧为 `0..100,000,000`，NPC 为 `0..50`。
- **FR-G7MAIN-03** SFMT 初始化、普通/ID offset、模 17 指针比较、QR 滑动窗口与 `ModelStatus` 时间换算只在独立 `gen7main` Wasm API v1 中执行。Seed 全空间检索使用最多 8 个独立 Worker，不依赖远程 API。
- **FR-G7MAIN-04** 提供进度、取消、确定顺序、CSV、结果清空、状态回写、错误和空结果状态；17 个 Clock 图片使用 3DSRNGTool 上游资源并随 PWA 本地缓存。

完整输入限制、算法、Worker/Wasm 契约、固定夹具和上游文件见 [Gen 7 Main RNG Tool](modules/gen7main.md)。

## 8.38 当前功能需求：`gen7eggseedfinder`

- **FR-G7EGGSEED-01** 提供 3DSRNGTool `Gen7EggSeedFinder` 的 127 鲤鱼王计算器与新存档 8 蛋性格检索两条工作流。
- **FR-G7EGGSEED-02** 鲤鱼王输入去除空白后必须恰好为 127 个 `0` / `1`；性格必须恰好为 8 项十进制 `0..25`；ShinyCharm 关闭/开启时每蛋后续推进基数分别为 `10` / `12`。
- **FR-G7EGGSEED-03** 默认搜索完整 `0x00000000..0xFFFFFFFF` Seed 空间；`Minimum Seed` / `Maximum Seed` 仅作为静态浏览器的本地分段与恢复控制。Worker Pool 最多使用 8 个独立 Wasm 实例，默认按 `2^20` Seed 分片，取消会终止并重建 Pool，并返回已经按顺序完成的前缀结果。
- **FR-G7EGGSEED-04** TinyMT、8 蛋生成顺序与 `MagikarpCalc` 127 x 127 GF(2) 逆矩阵只在独立 `gen7eggseedfinder` Wasm API v1 中执行。逆矩阵输出按 TinyMT 的 `31 + 32 + 32 + 32` 位状态布局打包，再按上游 `state3,state2,state1,state0` 顺序显示四个 8 位大写十六进制字。
- **FR-G7EGGSEED-05** 提供进度、取消、CSV、错误、空结果和三语界面；3DSRNGTool 已有简体中文词条逐字复用，没有来源的新增范围控件保留 English label。
...
## 8.40 当前功能需求：`gen8event`

- **FR-G8EVENT-01** 提供 PokeFinder `Gen 8 Event` Generator，仅接受 Brilliant Diamond / Shining Pearl Profile。Seed 0/1 使用最多 16 位的 64 位十六进制且空值读取为 `0`；两项同时为 `0` 时拒绝。Initial Advances、Max Advances、Offset 使用十进制 `uint32_t`，默认分别为 `0`、`100000`、`0`，三项之和不得溢出。
- **FR-G8EVENT-02** 支持物种 `1..493`、等级 `1..100`、五种 PID Type、五种配信特性、三种固定性别、性格锁定、TID/SID、EC、PID、`0..3` 个保底 31 IV 和蛋标记；严格解析 732 字节 `.wb8`，并以八位十六进制回填 EC/PID 和回填 Level。
- **FR-G8EVENT-03** 支持 Any / Star / Square / Star/Square 异色筛选、Any / Male / Female 性别筛选、Any / `0` / `1` / `H` 特性筛选，以及性格、觉醒属性、身高、体重和六项 IV 闭区间筛选。PokeFinder `EventGenerator8` 的 PID Type 分支缺陷按界面语义修正，不复制未初始化 PID 行为。
- **FR-G8EVENT-04** 外层 Xorshift、EC/PID、保底 IV、特性、性别、性格、身高、体重、能力值和筛选只在独立 `gen8event` Wasm API v1 中执行。请求固定为 45 个 `uint32_t`，结果固定为 11 个 `uint32_t`；单次任务最多评估 250,000,000 个状态并返回 100,000 行，Worker Pool 最多使用 8 个独立单线程实例。
- **FR-G8EVENT-05** 结果保留 PokeFinder `StaticModel8` 的 Advances、EC、PID、Shiny、Nature、Ability、HP、Atk、Def、SpA、SpD、Spe、Gender、Height、Weight、Characteristic 16 列；六项能力列可切换 IV 或能力值，支持虚拟滚动、排序、CSV、进度、取消、清空、错误、空结果和结果上限状态。桌面优先将档案、RNG 信息、设置/筛选和结果表收纳在首屏，`1280px` 以下重排为单栏。

完整输入限制、`.wb8` 字段、PID Type 缺陷修正、45/11-word ABI、固定夹具和上游文件见 [Gen 8 Event](modules/gen8event.md)。

## 8.41 当前功能需求：`gen8raids`

- **FR-G8RAIDS-01** 提供 PokeFinder `Gen 8 Raids` Generator，仅接受 Sword / Shield Profile。Seed 使用最多 16 位的 64 位十六进制且空值读取为 `0`，全零 Seed 拒绝；Initial Advances、Max Advances、Offset 使用十进制 `uint32_t`，默认分别为 `0`、`100`、空，空值读取为 `0`，三者之和不得溢出。
- **FR-G8RAIDS-02** 支持 Wild Area、Isle of Armor、Crown Tundra 的 Normal / Rare 巢穴和 69 张 Wild Area Event 表；静态数据固定为 197 张巢穴表、276 个地图巢穴映射与 1192 条 `personal_swsh.bin` 记录。普通巢穴 Level 为 `1..100` 且默认 `1`，Event 使用模板等级并禁用 Rarity。
- **FR-G8RAIDS-03** 模板只读显示 Ability `1 / 2 / H / 1/2 / 1/2/H`、Gender `Random / ♂ / ♀ / Genderless`、七种 Gender Ratio、IV Count、Shiny Type 与 Gigantamax；筛选严格保留 Any / Star / Square / Star/Square、Any / ♂ / ♀、Any / `0` / `1` / `H`，以及性格、身高、体重和六项 IV 闭区间。上游禁用的 Hidden Power 控件不显示。
- **FR-G8RAIDS-04** Xoroshiro、EC/PID、异色修正、保底 IV、特性、性别、性格、身高、体重、能力值和筛选只在独立 `gen8raids` Wasm API v1 中执行。请求固定为 41 个 `uint32_t`，结果固定为 12 个 `uint32_t`；单次任务最多评估 250,000,000 个状态并返回 100,000 行，Worker Pool 最多使用 8 个独立单线程实例。
- **FR-G8RAIDS-05** 结果保留 PokeFinder `StaticModel8` 的 Advances、EC、PID、Shiny、Nature、Ability、HP、Atk、Def、SpA、SpD、Spe、Gender、Height、Weight、Characteristic 16 列；六项能力列可切换 IV 或能力值，支持虚拟滚动、排序、CSV、进度、取消、清空、错误、空结果和结果上限状态。

完整输入限制、数据生成、41/12-word ABI、固定夹具和上游文件见 [Gen 8 Raids](modules/gen8raids.md)。

## 8.42 当前功能需求：`gen8static`

- **FR-G8STATIC-01** 提供 PokeFinder `Gen 8 Static` Generator，仅接受 Brilliant Diamond / Shining Pearl Profile。PokeFinder 没有该模块的 Searcher，也没有 Sword / Shield Static；Web 端不自行扩展。
- **FR-G8STATIC-02** Seed 0/1 使用最多 16 位的 64 位十六进制，空值读取为 `0`，两项同时为 `0` 时拒绝。Initial Advances、Max Advances、Offset 使用十进制 `uint32_t`，默认分别为 `0`、`100000`、空；Max Advances 包含起点，实际处理 `N + 1` 帧，三项之和不得溢出。
- **FR-G8STATIC-03** 支持 9 个上游分类与 47 个模板，按当前 BD/SP Profile 过滤版本；支持 None、25 种 Synchronize Nature、Cute Charm `♂ Lead` / `♀ Lead`，固定性别模板隐藏 Cute Charm。Level、Ability、Shiny 与 IV Count 只读。
- **FR-G8STATIC-04** 支持 Any / Star / Square / Star/Square 异色筛选、Any / Male / Female 性别筛选、Any / `1` / `2` / `H` 特性筛选，以及性格、身高、体重和六项 IV 闭区间。上游禁用的 Hidden Power 与 Wild 筛选不显示。
- **FR-G8STATIC-05** 普通定点使用 Xorshift 与 32 项 RNGList；Roamer 使用 Xorshift 生成 EC 后进入 XoroshiroBDSP。算法只在独立 `gen8static` Wasm API v1 中执行，请求固定为 41 个 `uint32_t`，结果固定为 11 个 `uint32_t`；单次任务最多评估 250,000,000 个状态并返回 100,000 行，Worker Pool 最多使用 8 个独立单线程实例。
- **FR-G8STATIC-06** 结果保留 PokeFinder `StaticModel8` 的 16 列；六项能力列可切换 IV 或能力值，支持虚拟滚动、排序、CSV、进度、取消、清空、错误、空结果和结果上限状态。

完整输入限制、模板分布、数据生成、41/11-word ABI、固定夹具和上游文件见 [Gen 8 Static](modules/gen8static.md)。

## 8.43 当前功能需求：`gen8underground`

- **FR-G8UNDERGROUND-01** 提供 PokeFinder `Gen 8 Underground` Generator，仅接受 Brilliant Diamond / Shining Pearl Profile。Seed 0/1 使用最多 16 位的 64 位十六进制，空值读取为 `0`，两项同时为 `0` 时拒绝；Initial Advances、Max Advances、Offset 使用十进制 `uint32_t`，默认分别为 `0`、`100000`、空，三项之和不得溢出。
- **FR-G8UNDERGROUND-02** 支持 BD/SP 各 18 个房间、Story Flag `1..6`、9 个固定 Level Flag 区间、Diglett Bonus 和当前房间物种多选；切换版本、Story Flag 或 Location 后按上游区域数据重建物种列表。
- **FR-G8UNDERGROUND-03** 支持 None、Cute Charm、Item Modifier、Level Modifier 与 25 种 Synchronize Nature 队首；支持 Any / Star / Square / Star/Square、Any / Male / Female、Any / `0` / `1`、性格、Hidden Power、身高、体重、六项 IV 与物种筛选。Hidden Power 控件按上游保持可见，但 `UndergroundStateFilter::compareState()` 不使用该值；Disable Filters 同时跳过物种筛选。
- **FR-G8UNDERGROUND-04** 房间选择、256 项 Xorshift RNGList、普通/特殊物种、EC/PID、异色修正、IV、特性、性别、性格、尺寸、道具、蛋招式、能力值和筛选只在独立 `gen8underground` Wasm API v1 中执行。请求固定为 54 个 `uint32_t`，结果固定为 12 个 `uint32_t`；单次任务最多评估 250,000,000 个状态并返回 100,000 行，Worker Pool 最多使用 8 个独立单线程实例。
- **FR-G8UNDERGROUND-05** 结果保留 PokeFinder `UndergroundModel` 的 20 列；六项能力列可切换 IV 或能力值，支持虚拟滚动、排序、CSV、进度、取消、清空、错误、空结果和结果上限状态。设置与筛选使用标签页，结果表是桌面主滚动区域。
...
## 8.46 当前功能需求：`gen6stationary`

- **FR-G6STATIONARY-01** 提供 3DSRNGTool Gen VI `Stationary RNG` 连续帧工作流，支持 X、Y、Omega Ruby、Alpha Sapphire、Transporter 与 `Gen6/PKM6.cs` 的全部 141 个目标模板。Seed 为 8 位十六进制且空值按 `0`；帧上游上限为 `1,000,000,000`，当前浏览器绝对帧保护上限为 `5,000,000`。
- **FR-G6STATIONARY-02** 支持模板 Delay、Consider Delay、Synchronize、Assume Synced、Shiny Charm、TSV/TRV、固定 IV、保底 3V/5V、Ability、Nature、Gender、Shiny Lock、Always Sync、Instant Sync、In-Game Trade OT TSV 与自定义目标。Transporter 支持最多 20 位 `0/1/2` GenderList 与目标编号。
- **FR-G6STATIONARY-03** MT19937、60 次非同步消耗、Delay、Bank / Pokemon Link / Transporter 前置消耗、EC/PID、异色与方块异色、IV、Ability、Nature、Gender、Hidden Power 和筛选只在独立 `gen6stationary` Wasm API v2 中执行。模块使用 49-word 请求、16-word 结果和单 Dedicated Worker。
- **FR-G6STATIONARY-04** 支持六项 IV 闭区间、25 种性格、16 种觉醒力量、完美 IV 值/数量、性别、特性、异色与方块异色筛选；结果保留 Frame、Random、EC、PID、六项 IV、Nature、Ability、Gender、Hidden Power、Shiny、Synchronize、Frame Used、PSV 与 PRV。
- **FR-G6STATIONARY-05** 提供进度、取消、100000 行结果上限、虚拟滚动、排序、CSV、清空、错误与空结果状态。桌面保持参数和结果双列工作区，窄屏重排为单列，结果表使用固定高度独立滚动区域。
...
## 8.50 当前功能需求：`gen6dexnav`

- **FR-G6DEXNAV-01** 提供独立的 3DSRNGTool Gen VI `DexNav RNG` 工作区，支持 Grass、Tall Grass 和 Surf 三种基础遇敌类型、主动搜索、图鉴导航遇敌、搜索等级、连锁长度、三种档案开关、Potential、Flute 和可编辑物种/等级槽位。
- **FR-G6DEXNAV-02** 按上游 `DexNav.cs` 复现 TinyMT 触发、摇晃坐标、槽位类型、Boost、Lead、Grade、等级/笛子修正、隐藏特性、潜力、蛋招式、持有物和闪光检查；保留上游 `FindPatch` / `PostCheck` 成功占位行为。
- **FR-G6DEXNAV-03** 算法只在独立 `gen6dexnav` Wasm API v1 中执行；模块使用 45-word 请求、16-word 结果和单 Dedicated Worker，支持进度、取消、100000 行结果上限、固定高度虚拟结果表、CSV、清空、错误和空结果状态。

完整输入限制、Grade/Boost/闪光检查顺序、45/16-word ABI、固定夹具和上游文件见 [Gen 6 DexNav](modules/gen6dexnav.md)。

## 8.51 当前功能需求：`gen6pokeradar`

- **FR-G6RADAR-01** 提供独立的 3DSRNGTool Gen VI `Poke Radar RNG` 工作区，支持 Tiny Seed/Frame、帧范围、Party Size、Chain Length、Boost 和结果上限。
- **FR-G6RADAR-02** 按上游 `PokeRadar.cs` 复现音乐状态、四个宝可梦块、GoodRate、一个空块、连锁/Boost 闪光阈值和 `Patch.X/Y` 坐标；提供 9×9 `B/G/S/X/C` 概览。
- **FR-G6RADAR-03** 算法只在独立 `gen6pokeradar` Wasm API v1 中执行；模块使用 8-word 请求、16-word 结果和单 Dedicated Worker，支持进度、取消、100000 行结果上限、固定高度虚拟结果表、CSV、清空、错误和空结果状态。

完整输入限制、GoodRate、坐标映射、8/16-word ABI、固定夹具和上游文件见 [Gen 6 Poke Radar](modules/gen6pokeradar.md)。

## 9. 后续实施顺序

1. PokeFinder 4.3.2 产品模块、3DSRNGTool Gen VII、Gen VI Stationary、Pokemon Link / Transporter、Event、Wild、DexNav、Poke Radar 与 Profile Manager 已实现。
2. 下一模块为 3DSRNGTool Gen VI Egg RNG，完成后继续 Gen VI 与其他公共工具库存。
...
23. GitHub Pages 在线加载四十个 Worker/Wasm 模块，控制台无资源、API 握手或 Worker 错误。
...
当前按完整模块库存推进。PokeFinder 4.3.2 产品模块、3DSRNGTool Gen VII、Gen VI Stationary、Pokemon Link / Transporter、Event、Wild、DexNav、Poke Radar 与独立 Profile Manager 已实现；下一模块为 Gen VI Egg RNG。
...
- **阶段 8C：`gen8egg` Gen 8 Eggs** - BDSP 双亲、遗传、异国孵化、护符、独立 Wasm/Worker 与上游夹具（已实现，待 Actions、部署回归与最终验收）。
- **阶段 8D：`gen8event` Gen 8 Event** - BDSP 配信 Generator、`.wb8` 导入、完整筛选、独立 Wasm/Worker 与上游夹具（已实现，待 Actions、部署回归与最终验收）。
...
- **阶段 9：3DSRNGTool** - Gen VII Stationary、Wild、SOS、Egg、Battle Tree、Event、ID、Main RNG Tool、Egg Seed Finder、Festival Plaza Facility RNG、Gen VI Stationary、Pokemon Link / Transporter、Event、Wild、DexNav、Poke Radar 与 Profile Manager 已实现；下一模块为 Gen VI Egg RNG，之后继续 Gen VI 与其他公共工具，仅 `NTR Helper` 排除。
