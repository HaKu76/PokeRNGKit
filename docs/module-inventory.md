...
| Gen IV     | Eggs、Event、IDs、Static、Wild、Profile Manager、IVs to PID、Seed to Time、Gen4SeedFinder、Search Coin Flips、Search Calls、Roamer Map、Chained Shiny to SID | Voltorb Flip Seed / Board Generator |
...
| T14  | MT 初始 Seed / Time Finder                             | 已实现（gen6mtseedtime）            |
...
| P3     | Gen IV 教程工具                    | `Swarm RNG` 与 `Gen4SeedFinder` 已实现；Voltorb Flip Seed/Board 仍待实现，并与现有 Gen4 Seed to Time、Advance、Static/Wild 去重 | Gen IV `Swarm.mdx`、`Voltorb Flip.mdx`、`Retail Initial Seed.mdx`                           |
...
| Gen 4                             | 34       | Static、Wild、Egg、ID、Advance、Seed to Time、Wondercard、Swarm、Gen4SeedFinder 已覆盖；Voltorb Flip 是剩余真实辅助缺口                               | 继续 P3，仅实现 Voltorb Flip，避免重复既有 Seed/Swarm 算法       |
