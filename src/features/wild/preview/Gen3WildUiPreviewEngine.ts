...
function previewState(
  ...
  let mixed =
    (request.seed ^
      Math.imul(advances, 0x045d_9f3b) ^
      Math.imul(request.area.slots.length, 0x9e37_79b9)) >>>
    0;
...
function matches(request: Gen3WildRequest, state: Gen3WildState) {
...
export class Gen3WildUiPreviewEngine implements Gen3WildSearchEngine {
  ...
  async search(
    ...
    if (this.running)
      throw new Error("A Gen3 wild UI preview is already running.");
    this.running = true;
