...
export interface Gen5StaticIvCache {
  identity: number;
  name: string;
...
export interface Gen5StaticShaCache {
  identity: number;
  name: string;
  initialAdvances: number;
  maxAdvances: number;
...
export function parseGen5StaticIvCache(
  buffer: ArrayBuffer,
  name: string,
): Gen5StaticIvCache {
...
export function parseGen5StaticShaCache(
  buffer: ArrayBuffer,
  name: string,
): Gen5StaticShaCache {
...
export function gen5StaticFastSearchEligible(
  request: Gen5StaticSearcherRequest,
  ivCache?: Gen5StaticIvCache,
) {
...
export function prepareGen5StaticCache(
  request: Gen5StaticSearcherRequest,
  ivCache?: Gen5StaticIvCache,
  shaCache?: Gen5StaticShaCache,
): Gen5StaticPreparedCache | undefined {
  if (!gen5StaticFastSearchEligible(request, ivCache) || !ivCache)
    return undefined;
  const type = request.template.roamer ? "roamer" : "normal";
  const pairs: number[] = [];
  ...
  if (shaCache && shaProfileCompatible(request, ivCache, shaCache)) {
    ...
    for (let offset = 0; offset < source.length; offset += 4) {
      ...
      if (
        ...
        isGen5StaticButtonMaskAllowed(request.profile, button)
      ) {
...
export function withGen5StaticCache(
