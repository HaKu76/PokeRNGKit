...
export function isGen3EggMethodValid(
  game: Gen3EggGame,
  method: Gen3EggMethod,
) {
  return game === "emerald"
...
export function validateGen3EggRequest(request: Gen3EggRequest): string[] {
  ...
  if (request.game === "rsfrlg" && (request.seedHeld > 0xffff || request.seedPickup > 0xffff)) {
    errors.push("seed");
  }
  if (
  ...
  if (request.compatibility !== 20 && request.compatibility !== 50 && request.compatibility !== 70) {
    errors.push("compatibility");
  }
  if (!Number.isInteger(request.species) || !GEN3_EGG_ALLOWED_SPECIES.has(request.species)) {
    errors.push("species");
  }
  ...
  for (const [name, parent] of [
    ...
    if (!Number.isInteger(parent.nature) || parent.nature < 0 || parent.nature > 24) {
      errors.push(`${name}Nature`);
    }
    for (const [index, iv] of parent.ivs.entries()) {
      if (!Number.isInteger(iv) || iv < 0 || iv > 31) errors.push(`${name}Iv${index}`);
    }
  }
  if (
...
export function createGen3EggChunks(
  ...
  if (!Number.isInteger(chunkSize) || chunkSize < 1 || chunkSize > GEN3_EGG_HELD_CHUNK_SIZE) {
    throw new RangeError(`Gen3 egg chunk size must be between 1 and ${GEN3_EGG_HELD_CHUNK_SIZE}.`);
  }
  const redrawCount =
    request.game === "emerald"
      ? request.maxRedraws - request.minRedraws + 1
...
export function decodeGen3EggStates(buffer: ArrayBuffer): Gen3EggState[] {
  ...
  for (let source = 0, target = 0; source < words.length; source += GEN3_EGG_RESULT_WORDS, target++) {
    states[target] = {
