...
当前 Gen VII Main RNG Tool 工作区包含：

- 读档 Clock 指针序列与 ID 指针序列的 32 位 Seed 反查、QR 指针帧定位和 Time Calculator
- Sun、Moon、Ultra Sun、Ultra Moon 起始帧、offset、NPC、Fidget 与 Raining 行为
- 独立 `gen7main` Wasm API v1、最多八个 Dedicated Worker、确定性分片、进度、取消和 CSV
- 输入边界、SFMT Seed 反查、QR 滑动窗口、时间换算和固定夹具见 [Gen 7 Main RNG Tool](docs/modules/gen7main.md)

当前 Gen VII Egg Seed Finder 工作区包含：

- 新存档 8 蛋性格全空间 Seed 检索，以及 127 个鲤鱼王性别结果反推四字 TinyMT 状态
- ShinyCharm 推进差异、`Minimum Seed` / `Maximum Seed` 本地分段恢复和最多八个独立 Worker
- 独立 `gen7eggseedfinder` Wasm API v1、`2^20` Seed 批次、确定顺序、进度、取消和 CSV
- 输入边界、127 位 GF(2) 逆矩阵、TinyMT 生成顺序和固定夹具见 [Gen 7 Egg Seed Finder](docs/modules/gen7eggseedfinder.md)

当前 Gen VII Festival Plaza 工作区包含：

- Sun、Moon、Ultra Sun、Ultra Moon 的 SFMT64 连续帧、NPC 眨眼模型、Delay 与 19 个 Rank 档位
- Stars、Facility、NPC Type、Color 筛选，以及 Moon 独立设施池、Switcheroo 版本限制和修正后的 `21-30` 星级概率
- 独立 `gen7festivalplaza` Wasm API v1、13/10+N-word 会话契约、单 Dedicated Worker、进度、取消、虚拟化结果表、排序与 CSV
- 输入边界、设施池、上游缺陷修正和固定夹具见 [Gen 7 Festival Plaza](docs/modules/gen7festivalplaza.md)
...
当前 Gen VIII Event 工作区包含：

- Brilliant Diamond / Shining Pearl 档案、两段 64 位 Seed、初始帧、最大帧数、Offset 与本地 `.wb8` 导入
- 配信物种、等级、PID Type、特性、性别、性格锁定、TID、SID、EC、PID、保底 IV、蛋标记和完整状态筛选
- 独立 `gen8event` Wasm API v1、Dedicated Worker Pool、250,000,000 次任务上限、100,000 行结果上限、进度、取消、虚拟结果表、排序与 CSV
- 输入边界、45/11-word 契约、上游缺陷修正和固定夹具见 [Gen 8 Event](docs/modules/gen8event.md)

当前 Gen VIII Raids 工作区包含：

- Sword / Shield 档案、Wild Area / Isle of Armor / Crown Tundra 普通与稀有巢穴，以及 69 张 Wild Area Event 表
...
当前 Gen VIII Wild 工作区包含：

- Brilliant Diamond / Shining Pearl 档案、双 64 位 Seed、推进、Offset、七种遭遇入口、地点、昼夜、宝可追踪、大量出现、丑丑鱼钓点、Great Marsh / Trophy Garden Replacement 与甜甜蜜树槽位
- None、Synchronize、Cute Charm、道具、等级和槽位修正队首，以及异色、性别、特性、性格、隐藏力量、槽位、等级、身高、体重和六项 IV 筛选
- 独立 `gen8wild` Wasm API v1、48/12-word 契约、Dedicated Worker Pool、虚拟结果表、排序、CSV、进度和取消
- 输入边界、数据生成、上游来源哈希和固定夹具见 [Gen 8 Wild](docs/modules/gen8wild.md)

当前 Gen VIII Den Map 工作区包含：

- Wild Area、Isle of Armor、Crown Tundra 三个区域和全部 276 个巢穴点位
- 复用 `GEN8_DEN_INFO` 的上游坐标、地点本地化、Special index `16` 和三张原图地图资源
- 区域/巢穴选择、坐标详情、响应式地图标记和独立地图滚动，不新增 Wasm 或 Worker
- 上游资源尺寸、SHA-256、输入边界和版权说明见 [Gen 8 Den Map](docs/modules/gen8denmap.md)
...
- **阶段 8：`gen7stationary`、`gen7wild`、`gen7sos`、`gen7egg`、`gen7battletree`、`gen7event`、`gen7main`、`gen7eggseedfinder`、`gen7festivalplaza`、`gen7id` 与 `pokerusfinder`** - 第七世代 Stationary / Wild / SOS / Egg / Battle Tree / Event / Main RNG Tool / Egg Seed Finder / Festival Plaza / ID Generator，以及 DevonStudios 第三/四世代宝可病毒帧查询；当前源码已实现，待 Actions、部署回归与最终验收。
